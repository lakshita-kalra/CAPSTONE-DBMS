...
// Valid credentials
const DEMO_USERS = {
  'analyst@cyberwatch.io': { password:'CyberWatch@2024', role:'SOC Analyst' },
  'lead@cyberwatch.io':    { password:'CyberWatch@2024', role:'IR Lead' },
  'ciso@cyberwatch.io':    { password:'CyberWatch@2024', role:'CISO' },
};
...
async function doLogin(){
  ...
  let authUser=null, authRole=selectedRole;

  try {
    // Try real DB first
    const dbUser = (typeof apiLogin === 'function') ? await apiLogin(user, pass) : null;
    if(dbUser){ authUser=dbUser.email||user; authRole=dbUser.role||selectedRole; }
    else {
      // Fallback: check demo credentials
      const demo = DEMO_USERS[user.toLowerCase()];
      if(demo && demo.password===pass){ authUser=user; authRole=demo.role; }
      else { toast('Access Denied','Invalid email or password','danger'); btn.textContent='AUTHENTICATE →'; btn.disabled=false; return; }
    }
  } catch(e) {
    const demo = DEMO_USERS[user.toLowerCase()];
    if(demo && demo.password===pass){ authUser=user; authRole=demo.role; }
  ...
  S.currentUser=authUser; S.currentRole=authRole; S.sessionStart=new Date();
